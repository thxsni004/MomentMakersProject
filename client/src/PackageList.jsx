    ...
    const [packages, setpackages] = useState([]);
      ...
      useEffect(() => {
        ...
        const fetchPackages = async () => {
          try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/admin/packages`);
      ...
      const handleDelete = async (id) => {
        try {
          await axios.delete(`${import.meta.env.VITE_API_URL}/admin/packages/${id}`);
      ...
      const handleEditPrice = async (id) => {
        ...
        try {
          const updatedPackage = await axios.put(`${import.meta.env.VITE_API_URL}/admin/packages/${id}`, {
            price: newPrice,
          });
...
{editingPackage === pkg._id ? (
            ...
            {pkg.image && (
               <img
               src={`${import.meta.env.VITE_API_URL}${pkg.image}`}  // Ensure correct URL
               alt={pkg.name}
