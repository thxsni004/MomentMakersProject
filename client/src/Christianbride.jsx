...
const[user,setselecteduser]=useState(()=>{
  return JSON.parse(sessionStorage.getItem("user"))||null;
    ...
    const[camera,setcamera]=useState([]); 
     ...
     // Save selections to sessionStorage whenever they change
      ...
      }, [selectedStage, selectedAdd, selectedCameraPackage, selectedSetting, numberOfPeople, foodPrice]);
  ...
  const clearAllSelections = () => {
    ...
    // ];
    // const stageOptions = [
    //   { id: "middle", name: "Middle Class Stage", price: 20000, image:"christian/stage2.jpg" },
    //   { id: "upperMiddle", name: "Upper Middle Class Stage", price: 60000,image:"christian/stage1.jpg" },
    //   { id: "high", name: "High Class Stage", price: 200000,image:"christian/stage.jpg"  },
    // ];
 ...
 sessionStorage.setItem("orderHistory", JSON.stringify(orderHistory));
    ...
    sessionStorage.setItem("cartData", JSON.stringify(orderDetails));
    
    // Navigate to cart page and pass state
    navigate("/cart", { state: { orderDetails } });
    
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/cbride`, {
          method: 'POST',
          headers: {
    ...
    useEffect(() => {
      const fetchStages = async () => {
        try {
          ...
          setStages(data); // Set the fetched stages in the state
        } catch (error) {
   ...
   <Breadcrumb className="custom-breadcrumb">
        ...
        <Breadcrumb.Item href="/chrstian">🔙 </Breadcrumb.Item>
      </Breadcrumb>
      <h2>Food Packages</h2>
